...
function connect(callback, options = null, att = 0) {
  ...
  try {
    client.connect((err => {
      if (err) {
        let errMessage = `[SQLCNXERR]|${err.code}|${err.message}`;
        console.error('%s', errMessage);

        // ensure we return a generic message outside of dev env
        if (process.env.NODE_ENV === 'production') {
          errMessage = 'Unable to connect to the database';
        }
