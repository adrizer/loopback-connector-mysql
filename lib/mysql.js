...
function connect(callback, options = null, att = 1) {
  const client = this.client || mysql.createConnection(options);
  // client.connector = mysql;
  client.connection = client;
  client.connect((err => {
    if (err) {
      console.error('ErrorMessage: [att=%s] %j', att, err);

      if (att <= this.maxConnectionAttempts) {
        return setTimeout(connect.bind(this, callback, options, att + 1), 2000 * 2 ** att);
      }

      const msg = formatErrorMsg(err);
      debug('%s', `[SQLCNXERR]|${msg}`);
...
/**
 * For query errors we get a slightly different error object structure (with no `message` field)
 * @param {mysql.QueryError & {sqlMessage?:string,sqlState?:string,sql?:string}} err
 */
function formatErrorMsg(err) {
  const msg = `${err.code} ${err.message || err.sqlMessage}`;
...
MySQL.prototype.client = null;

MySQL.prototype.maxConnectionAttempts = parseInt(process.env.LB_MAX_CNX_ATT, 10) || 3;
MySQL.prototype.maxQueryAttempts = parseInt(process.env.LB_MAX_QUERY_ATT, 10) || 3;
...
MySQL.prototype.connect = function(callback) {
  ...
  if (this.client) {
    ...
    connect.call(self, callback, options);
  }
};
...
function generateOptions(settings) {
  ...
  if (s.url) {
    ...
    /**
     * per this warning: "Ignoring invalid configuration option passed to Connection: ${...} .
     ...
     */
    const bl = ['name', 'connector', 'collation', 'username', 'createDatabase', 'lazyConnect'];
    ...
    for (const p in s) {
      ...
      if (options[p] === undefined && !bl.includes(p)) {
        options[p] = s[p];
      }
    }
  }
...
MySQL.prototype.executeSQL = function(sql, params, options, callback) {
   ...
   */
  function runQuery(connection, att = 1) {
    connection.query(sql, params, function (err, data) {
      ...
      if (err && att <= this.maxQueryAttempts) {
        return setTimeout(() => {
          try { this.client.end(); } finally { this.client = null; }
          this.connect(() => runQuery(connection, att + 1));
        }, att * 2000 ** 2);
      }
  ...
  if (transaction && transaction.connection &&
    ...
    connect.call(self, executeWithConnection, generateOptions(this.settings));
  }
};
...
MySQL.prototype.escapeName = function(name) {
  return (this.client || mysql.createConnection(generateOptions(this.settings))).escapeId(name);
};
